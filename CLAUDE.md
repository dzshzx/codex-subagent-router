...
- 公共 API 只经 `codex_subagent_router/__init__.py` 暴露：policy / protocol value types 与解析编码函数、validator、role source、start-context / document handlers、skill 渲染（`render_skill_markdown` / `skill_name`）、usage report value types 与 `usage_report`，以及安装 value types 和 plan / install / status / rollback 函数。Hook 规格、命令 wiring、receipt 格式、派发指导文本源和安装/回滚辅助实现保持私有。
