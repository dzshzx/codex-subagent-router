...
- `src/codex_subagent_router/policy.py` 是**唯一策略事实源**：私有的不可变 `_ROUTING_POLICY` 分别保存按能力升序排列的 model catalog 与按推理深度排列的 effort catalog；支持集合由此派生。协议相关输出必须从这一处派生，不得出现第二策略源、固定 model/effort 配对表或隐藏 fallback。
...
- `validator.py` 只做 routed `spawn_agent` 的 deny-only 校验；合法或非 spawn 调用不产生 Hook 输出，不合法调用返回明确 deny，且 model / effort 必须从 `policy.py` 独立校验，不得引入配对 allowlist 或静默 rewrite。
- `roles.py` 是两个 managed identity 合同和 description 的唯一可执行来源；合同只描述稳定行为，不包含 model、effort、service tier 或 fork 配置。`start_context.py` 从 policy、validator 与 role 来源派生 root `startup` guidance，并只为 managed identity 注入精确合同。
- `document_handlers.py` 把严格解析、对应的纯 handler 和编码组合成三个 JSON document adapter；event 类型不匹配时抛出 `ProtocolViolation`，无输出时返回空字符串。
- `skill_source.py` 是派发指导文本（委派信号、任务包模板、结果契约）的唯一来源；`skill_render.py` 从 policy、roles、validator 与 skill_source 派生生成 agent-skill markdown（`render-skill`），输出是生成物、不得手改或成为第二策略源。`usage_report.py` 对显式 sessions 目录只读扫描 rollout 文件，提取 spawn 调用并重放 deny-only validator，输出路由分布与违规率。
...
- `install_commands.py` 是项目统一薄 CLI：安装操作要求 `--codex-home`，为 plan / install / status / rollback 输出机器可读 JSON 和明确退出码，不自行推断 Codex home；另提供 `render-skill`（生成 skill 文档）与 `usage-report`（要求显式 `--sessions-dir`）。
- 公共 API 只经 `codex_subagent_router/__init__.py` 暴露：`RoutingPolicy`、model/effort option、`routing_policy()`、`validate_routed_compute()`、protocol value types 与解析编码函数、validator、role source、start-context / document handlers、skill 渲染（`render_skill_markdown` / `skill_name`）、usage report value types 与 `usage_report`，以及安装 value types 和 plan / install / status / rollback 函数。Hook 规格、命令 wiring、receipt 格式、派发指导文本源和安装/回滚辅助实现保持私有。
- model catalog 与 effort catalog 各自按能力和推理深度升序排列；测试锁定两个独立 catalog 的精确顺序——改动即改动契约。
