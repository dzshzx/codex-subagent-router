...
- `src/codex_subagent_router/policy.py` 是唯一策略事实源。私有的不可变
  ...
  排列的 effort catalog；支持集合从中派生，不得新增第二策略源、固定
  model/effort 配对表或隐藏 fallback。
...
- `src/codex_subagent_router/roles.py` 是两个 managed identity 合同的唯一可执行
...
- `src/codex_subagent_router/__init__.py` 是公共 API 边界，对外暴露
  `RoutingPolicy`、model/effort option、`routing_policy()`、
...
- 使用清晰的领域命名，如 `RoutingPolicy`、`ModelOption`、
...
- 协议输出必须从唯一策略事实源派生，不得复制 model/effort catalog 或
  增加固定配对 allowlist。
- managed identity 合同与 description 只能来自 `roles.py`；Hook event、matcher、
