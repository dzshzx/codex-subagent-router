...
帖子所述 Sol/Terra/Luna 的模型目录 V1/V2 映射可能描述其成文时的另一版本或服务端目录，不能
外推为 0.144.4 合同。在 `rust-v0.144.4`，feature declaration 将 `multi_agent` 标为稳定默认
启用，将 `multi_agent_v2` 标为 under-development 默认关闭，参见
[`features/src/lib.rs`](https://github.com/openai/codex/blob/rust-v0.144.4/codex-rs/features/src/lib.rs#L1035-L1046)；
本机 `codex-cli 0.144.4` 也显示 hooks/multi_agent stable enabled、multi_agent_v2
under-development disabled。当前本机配置虽预置
`[features.multi_agent_v2] hide_spawn_agent_metadata=false, tool_namespace="agents"`，但未设置
`enabled=true`，所以这些子项不等于启用 V2。
...
| 0.144.4 feature 状态 | stable/enabled | under-development/disabled |
| 本项目状态 | 已用 installed binary 验证的支持面 | 仅 loopback preview arm；未验证真实后端 |
...
缺少必要输入而 fail closed。故本项目的 0.144.4 正式兼容面应继续写成“Hook-managed stable
V1 path”，而不是因为 loopback V2 可运行就扩大承诺。
...
同时维持当前边界：0.144.4 正式支持面只写 stable V1 Hook-managed path；四个 managed 名在所有
active custom-agent layer 中保留；project 冲突由 operator 负责；V2 只保留 preview/loopback
证据；Hook enforcement 明示 fail open。
